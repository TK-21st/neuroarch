...
class LarvaLoader(object):
    ...
    def load_neurons(self, file_name, morph_dir):
        ...
        for i, neuron in df.iterrows():
            ...
            if any([k in neuron.name for k in LN_keys]):
                locality = True
            else:
                locality = False

            # Create Neuron Node
            n = self.g_orient.Neurons.create(name=neuron.name, locality=locality)
            self.logger.info('created node: {0}({1})'.format(n.element_type, n.name))
            
            # Create Neurotransmitter Node if required
            nt = None
            neurotransmitter = []
            for key in LarvaLoader.neurotransmitter_map:
                if neuron[0].startswith(key):
                    neurotransmitter.append(LarvaLoader.neurotransmitter_map[key])
            if neurotransmitter:
    ...
    def load_neurons(self, file_name,morph_dir):
        ...
        with open(file_name, 'rb') as csvfile:
            ...
            for neuron in reader:
                ...
                # Check if neuropil exists
                npl = self.g_orient.Neuropils.query(name=LarvaLoader.neuropils[neuron[4]][0]).first()
                if not npl:
                    npl = self.g_orient.Neuropils.create(\
                                            name=LarvaLoader.neuropils[neuron[4]][0],
                                            synonyms=LarvaLoader.neuropils[neuron[4]][1])
                    self.logger.info('created node: {0}({1})'.format(npl.element_type, npl.name))
